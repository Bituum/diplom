...
public class AdminDishesController {
    ...
    public String createDish(Model model,
        ...
        listI = productsService.getAllProductsEntities().stream().map(ProductsEntity::getId).collect(Collectors.toList());
        listStr = productsService.getAllProductsEntities().stream().map(ProductsEntity::getProductDescription).collect(Collectors.toList());
    ...
    public String createDish(Model model,
        ...
        if (!validation.validate(time)) {
            model.addAttribute("TIME_ERROR", 1);
            List<Integer> listI = new ArrayList<>();
            List<String> listStr = new ArrayList<>();
            listI = productsService.getAllProductsEntities().stream().map(ProductsEntity::getId).collect(Collectors.toList());
            listStr = productsService.getAllProductsEntities().stream().map(ProductsEntity::getProductDescription).collect(Collectors.toList());
